...
PROMPT_GENERATOR_PROMPT = """
Reformat the following user-provided music details into a simple comma-separated list of audio tags.

User Input: "{user_prompt}"

Follow these guidelines strictly when reformatting:
- **Parse the "User Input" thoroughly.** It may contain a general music description, or it might explicitly define specific attributes like "Genre:", "Vocal Type:", "Instruments:", "Mood:", "Tempo:", or "Key:".
- **If an attribute is explicitly defined** in the "User Input" (e.g., "Genre: electronic"), **use that value directly** as a tag.
- **If an attribute is NOT explicitly defined**, **infer a tag for that category from the general music description** provided within the "User Input".
- **Always include at least one tag from each of these categories** in your final list:
    - **Genre** (e.g., "rap", "pop", "rock", "electronic", "classical")
    - **Vocal type** (e.g., "male vocal", "female vocal", "spoken word", "choir", "no vocal")
    - **Instruments** (e.g., "guitar", "piano", "synthesizer", "drums", "orchestral strings", "percussion")
    - **Mood/Energy** (e.g., "energetic", "calm", "aggressive", "melancholic", "uplifting", "driving")
    - **Tempo** (e.g., "120 bpm", "fast tempo", "slow tempo", "moderate tempo", "ambient tempo")
    - **Key** (e.g., "major key", "minor key", "C major", "A minor", "no specific key")
- The output must be a single line of comma-separated tags. **Do not add any other text or explanation.**
- **Example Output Format:** melodic techno, male vocal, electronic, emotional, minor key, 124 bpm, synthesizer, driving, atmospheric

If the combined tags (from explicit user input and inference) are few (less than 6 unique tags), infer what the user wants and add 2-3 more tags that are synonyms or closely related to the existing tags. **Do not introduce new categories** when adding synonyms.
